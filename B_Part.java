...
public class LeitourgikaB {
	...
	static ArrayList <Integer> used1 = new ArrayList<Integer>();			//An static array list for used numbers. 											
	static ArrayList <Integer> used2 = new ArrayList<Integer>();			//An static array list for used numbers. 																
	static Semaphore mutex = new Semaphore(1);								//Semaphore methon for 1 Thread. 										
	...
	public static void main(String[] args) 
	{
		...
		{
		  int numbers = random.nextInt(100 -(-100) + 1) + (-100); 			//These numbers are limited integers, which belongs to (-100,100) field.																																						
	...
	public static class T1 extends Thread{
		...
		{ 
			try 
			{
				mutex.acquire();															//Mutex acquire Thread 1 (T1).	
				System.out.println("Thread 1 got the permit.");								//Permit Message for Thread 1 (T1) appears.						
				
				try 
				{
					System.out.println("Starting Thread T1.");            					//Thread 1 (T1) start message appears at the console screen.																	
					Random random = new Random();						 					//A random-type variable, random.																
					boolean new_numbers = true;												//A boolean-type variable, new_numbers							
					int count = 0;													    	//An int-type variable, count.	
					
					while (new_numbers)                               						//While new_numbers is true.
					{
						int randomNumber = random.nextInt(array.length);            	    //A random number from the array is selected.
						...
						if (!used1.contains(randomNumber))									//The new number adds to the array when is not duplicate.		
						{
							used1.add(randomNumber);										//Adds to an array list the array's spot where number will change.	
							count++;														//count = count + 1.			
							array[randomNumber] = numbers;									//Random spot from the array changes with the randomly selected number.											
						}
						
						if (count >= array.length/2) 										//If count is over the half of the elements of the array.						
						{
							new_numbers = false;											//Then new_numbers is false.					
						}
					}
						System.out.println("Thread 1: " + Arrays.toString(array));			//The new array is printed at the screen.												
						System.out.println("Thread 1 changed spots: " + used1);				//The array's spots which changed, appears at the console screen.				
				...
				finally 
				{
					...
					System.out.println("Thread 1 releasing lock.");							//Thread 1 (T1) releases lock message appears at the console screen.		
					mutex.release();														//Mutex release Thread T1 after T1's process is completed and now is empty.			
				}
			}
	...
	public static class T2 extends Thread{
		...
		{ 
			try 
			{
				mutex.acquire();                                       				    	//Mutex acquire Thread 2 (T2).	
				System.out.println("Thread 2 got the permit.");         					//Permit Message for Thread 2 (T2) appears.
				...
				try 
				{
					...
					{                              					
						...
						if (!used2.contains(randomNumber))									//The new number adds to the array when is not duplicate.											
						{															
							used2.add(randomNumber);										//Adds to an array list the array's spot where number will change.								
							count++;														//count = count + 1.																		
							array[randomNumber] = numbers;									//Random spot from the array changes with the randomly selected number.																	
						}														
						...
						System.out.println("Thread 2: " + Arrays.toString(array));			//The new array is printed at the screen.																
						System.out.println("Thread 2 changed spots: " + used2);				//The array's spots which changed, appears at the console screen.			
				...
				finally 
				{
					...
					System.out.println("Thread 2 releasing lock.");							//Thread 2 (T2) release message appears at the console screen.									
					mutex.release();														//Mutex release Thread T2 after T2's process is completed and now is empty.		
				}
			}
			catch(InterruptedException e)													//Exception.				
			{
